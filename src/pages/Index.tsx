...
export default function Index() {
  ...
  useEffect(() => {
    supabase.from('feedbacks').select('id, setor, status, criado_em, autor').then(({ data }) => {
      if (data) setFeedbacks(data);
    });
    supabase.from('funcionarios').select('id, nome, cargo, departamento, foto_url, feedbacks_recebidos, feedbacks_resolvidos').then(({ data }) => {
      if (data) setFuncionarios(data as DashboardFuncionario[]);
    });
  }, []);
  ...
  // Distribution by setor from real data
  const setorCounts: Record<string, number> = {};
  feedbacks.forEach(f => {
    setorCounts[f.setor] = (setorCounts[f.setor] || 0) + 1;
  });
  const setorData = Object.entries(setorCounts).map(([key, count]) => ({
    label: setorLabels[key as FeedbackSetor] || key,
    count,
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => navigate('/cadastro')} className="text-sm text-primary hover:underline">Ver todos</button>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {funcionarios.map((func, i) => {
            const pct = func.feedbacks_recebidos > 0
              ? Math.round((func.feedbacks_resolvidos / func.feedbacks_recebidos) * 100)
              : 0;
            return (
              <motion.div
                key={func.id}
              ...
              >
                {func.foto_url ? (
                  <img
                    src={func.foto_url}
                    alt={func.nome}
                    className="w-14 h-14 rounded-full mx-auto mb-2 object-cover border-2 border-primary/20"
                  />
                ) : (
                  <div className="w-14 h-14 rounded-full mx-auto mb-2 bg-primary/10 flex items-center justify-center text-primary font-bold text-lg">
                    {func.nome.charAt(0)}
                  </div>
                )}
                <p className="font-semibold text-sm truncate">{func.nome}</p>
                <p className="text-xs text-muted-foreground truncate">{func.cargo}</p>
                <p className="text-xs text-muted-foreground truncate mb-2">{func.departamento}</p>
                <div className="flex justify-center gap-3 text-xs">
                  <div>
                    <p className="font-bold text-foreground">{func.feedbacks_recebidos}</p>
                    <p className="text-muted-foreground">Receb.</p>
                  </div>
                  <div>
                    <p className={`font-bold ${pct >= 70 ? 'text-success' : pct >= 40 ? 'text-warning' : 'text-destructive'}`}>{pct}%</p>
                    <p className="text-muted-foreground">Resolv.</p>
                  </div>
                </div>
              </motion.div>
            );
