...
export default function FeedbackDetail() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* â”€â”€ Status Progress Pipeline â”€â”€ */}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }} className="corporate-kpi">
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">SLA (â‰¤15d)</p>
          <p className={`text-2xl font-bold mt-1 ${daysOpen <= 15 || isResolved ? 'text-success' : 'text-destructive'}`}>
            {isResolved ? (daysOpen <= 15 ? 'âœ“ OK' : 'âœ— Estourado') : (daysOpen <= 15 ? 'No prazo' : 'Atrasado')}
          </p>
        </motion.div>
