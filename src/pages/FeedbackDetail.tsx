...
import { ArrowLeft, ThumbsUp, MessageSquare, Calendar, User, Building2, UserCheck } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card rounded-xl p-6">
        ...
        <div className="space-y-4">
          {feedback.comentarios > 0 ? (
            <div className="bg-muted/50 rounded-lg p-4">
              ...
              <p className="text-sm text-muted-foreground">Estamos analisando essa questão.</p>
            </div>
          ) : (
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <textarea placeholder="Adicionar um comentário..." className="w-full bg-muted rounded-lg p-3 text-sm outline-none resize-none placeholder:text-muted-foreground min-h-[80px]" />
          <button className="mt-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">Comentar</button>
        </div>
      </motion.div>
    </div>
  );
