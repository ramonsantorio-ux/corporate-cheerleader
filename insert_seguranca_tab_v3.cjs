...
// Use regex to ignore \r or spaces between elements
const triggerRegex = /(<TabsTrigger value="dre"[^>]*><DollarSign[^>]*\/>DRE Detalhada<\/TabsTrigger>\s*<\/TabsList>)/;
if (triggerRegex.test(code)) {
    ...
    code = code.replace(triggerRegex, triggerReplace);
    console.log('Trigger replaced successfully!');
} else {
    console.log('Could not find triggerSearch block');
}

const contentRegex = /(<\/TabsContent>\s*<\/Tabs>\s*)\)( \: \()/;
if (contentRegex.test(code)) {
    const contentReplace = `</TabsContent>
    ...
    code = code.replace(contentRegex, contentReplace);
